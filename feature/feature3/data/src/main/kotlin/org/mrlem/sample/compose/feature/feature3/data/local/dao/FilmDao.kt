...
import org.mrlem.sample.compose.feature.feature3.data.local.entity.FilmEntity
...
interface FilmDao {
    ...
    @Query("SELECT * FROM film3")
    fun list(): Flow<List<FilmEntity>>

    @Query("SELECT * FROM film3 WHERE id = :id")
    ...
    @Query("UPDATE film3 SET isFavorite = 1 WHERE id = :id")
    suspend fun favorite(id: String)

    @Query("UPDATE film3 SET isFavorite = 0 WHERE id = :id")
